...
Private Function ConstruirXml(anio As Long) As String
    ...
    s = s & Linea("nombre", SaneoNombre(DatoXml("Nombre del declarante")))
    ...
    If nIdCony <> "" Then
        ...
        s = s & Linea("nombreCony", SaneoNombre(DatoXml("Nombre del conyuge")))
    End If
...
Private Function ModuloXml(key As String, mapa As Variant, anio As Long, _
    ...
    Do While r < 5000
        ...
        If valor > 0 Then total = total + valor    ' SRI: solo positivos
        If valor > 0 Then
                ...
                Else
                    ...
                    ElseIf celda <> "" Then
                        ...
                        Select Case etiqueta
                            Case "nombreDeudor", "nombreAcreedor", _
                                 "nombreIfiExterior", "nombempresa", _
                                 "descripcion"
                                valor2 = SaneoNombre(valor2)
                        End Select
...
Private Function SumaModulo(hoja As String, anio As Long) As Double
    ...
    Do While r < 5000
        If CStr(ws.Cells(r, 1).Value) = "TOTAL" Then Exit Do
        Dim vmod As Double: vmod = ValTexto(CStr(ws.Cells(r, colVal).Value))
        If vmod > 0 Then t = t + vmod    ' SRI: solo valores positivos
        r = r + 1
    Loop
...
' Limpia nombres dejando solo letras, digitos, espacios y enie (SRI).
Private Function SaneoNombre(s As String) As String
    Dim out As String, res As String, i As Long, c As String, k As Long
    out = Replace(s, "&", " Y ")
    For i = 1 To Len(out)
        c = Mid$(out, i, 1)
        k = AscW(c)
        If (k >= 48 And k <= 57) Or (k >= 65 And k <= 90) Or _
           (k >= 97 And k <= 122) Or k = 32 Or k = 241 Or k = 209 Then
            res = res & c
        Else
            res = res & " "
        End If
    ...
    Loop
    res = Trim$(res)
    If Len(res) > 60 Then res = Left$(res, 60)    ' SRI: maximo 60 caracteres
    SaneoNombre = res
End Function
