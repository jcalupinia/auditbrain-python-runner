...
| A23 | **Motor con condiciones, fechas y tramos, y fichas con programa y requerimientos propios.** Reglas `gt`/`gte`/`lt`/`lte`/`eq` (1 o 0), `if` (tercer operando `c`), `days` (entre fechas o hasta `corte` = `parameters.cutoff`) y `band` (tramos) en `domain.mjs`, `audit_engine.py` y el motor portátil, con fórmula visible en el Excel (`IF`, `LOOKUP`) y en las explicaciones. Una definición puede traer `program`, `requests` y `source_pymes`; `createProgram`/`createRequests` los usan en vez de los genéricos. Motivo: el encargo de pruebas NIIF a ChatGPT (pérdida crediticia por antigüedad y similares) | `973a5d1` | ✅ `tests/tools` 78/78 · suite 107/107 · 4 mutaciones del motor Python detectadas · `tsc` sin errores · verif-niif16 y verif-alcance pasan · checkout sincronizado (78/78 allí) |
| A24 | **Estado del papel en la portada y las cédulas.** Una prueba en curso salía como «COPIA DEL PAPEL APROBADO»; ahora dice «PAPEL EN PROCESO — PENDIENTE DE REVISIÓN Y APROBACIÓN». El borrador de ChatGPT conserva su aviso de fuentes no verificadas | `c50fb5a` | ✅ tools 81/81 · suite 110/110 · checkout sincronizado |
| A25 | **Base técnica del catálogo.** VNR y PCE traen resumen de la norma, referencia PYMES (Secc. 13/27 y Secc. 11), marco al que sirve cada una (`frameworks`: PCE solo NIIF completas) y sus NIA con párrafos y qué exige cada una en la prueba; la cédula 04 las lista | `05dc20a` | ✅ tools 82/82 · suite 111/111 · `tsc` sin errores · checkout sincronizado |
