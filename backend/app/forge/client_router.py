...
from .router import _memory_out
from .schemas import (
    BrainCreate,
    BrainOut,
    CompileOut,
...
@client_router.get("/brains/{brain_id}/memory", response_model=list[MemoryOut])
def list_memory(
    brain_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(require_forge_client),
...
@client_router.post(
    "/brains/{brain_id}/memory", response_model=MemoryOut, status_code=201
)
def add_memory(
    brain_id: int,
