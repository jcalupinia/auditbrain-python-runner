...
export function workbookSheets(t,template=false){
 ...
 const constants={};for(const r of d.rules)for(const a of [r.a,r.b,r.c])if(typeof a==='string'&&a.startsWith('#')&&!constants[a]){constants[a]=s[2].length+1;s[2].push([`Constante ${a}`,numeric(a.slice(1)),'Constante de metodología']);}
 ...
 const ref=a=>a==='corte'&&!d.fields.some(f=>f.key==='corte')?`'03_Parametros'!$B$5`:a.startsWith('#')?`'03_Parametros'!$B$${constants[a]}`:a==='rate'&&d.id==='pce'?`C${n}`:d.fields.some(f=>f.key===a)?`'06_Data_Procesada'!${col(d.fields.findIndex(f=>f.key===a))}${n}`:`${col(ruleOffset+d.rules.findIndex(r=>r.key===a))}${n}`;
 d.rules.forEach(r=>{const a=ref(r.a),b=r.b===undefined?'':ref(r.b),c=r.c===undefined?'':ref(r.c);const exp={add:`${a}+${b}`,subtract:`${a}-${b}`,multiply:`${a}*${b}`,divide:`${a}/${b}`,min:`MIN(${a},${b})`,max:`MAX(${a},${b})`,gt:`IF(${a}>${b},1,0)`,gte:`IF(${a}>=${b},1,0)`,lt:`IF(${a}<${b},1,0)`,lte:`IF(${a}<=${b},1,0)`,eq:`IF(${a}=${b},1,0)`,if:`IF(${a}<>0,${b},${c})`,days:`(${b}-${a})`,band:r.op==='band'?`LOOKUP(${a},{${r.table.map(x=>x.from).join(',')}},{${r.table.map(x=>x.value).join(',')}})`:''}[r.op];line.push(formula(`IF(COUNTA('05_Data_Original'!A${n}:${col(d.fields.length-1)}${n})=0,"",IF(COUNTA('05_Data_Original'!A${n}:${col(d.fields.length-1)}${n})<${d.fields.length},NA(),ROUND(ROUND(${exp},6),${r.precision})))`,template?'':run.rows[i]?.[r.key]??''));});s[6].push(line);
 });
