...
// Base técnica de las pruebas del catálogo: resumen de la norma, referencia
// PYMES y las NIA que rigen la prueba, con qué exige cada una en ella.
const PYMES='https://www.ifrs.org/issued-standards/ifrs-for-smes/';
const NIA_VNR=[
 {document:'NIA 501',section:'párr. 4–8',requirement:'Si los inventarios son materiales, asistir al recuento físico y cotejar cantidades con los registros: existencia y condición.'},
 {document:'NIA 540 (Revisada)',section:'párr. 13 y 17–30',requirement:'El VNR es una estimación contable: evaluar el método, los datos (precios y costos) y los supuestos significativos, y el posible sesgo de la dirección.'},
 {document:'NIA 500',section:'párr. 6–9',requirement:'Evidencia suficiente y adecuada; evaluar la exactitud e integridad del inventario valorado que produce la entidad.'},
 {document:'NIA 330',section:'párr. 18',requirement:'Aplicar procedimientos sustantivos a la valoración del inventario por ser un saldo material.'},
];
const NIA_PCE=[
 {document:'NIA 540 (Revisada)',section:'párr. 13 y 17–30',requirement:'La pérdida crediticia esperada es una estimación: evaluar la matriz de provisión, las tasas por tramo y la información prospectiva.'},
 {document:'NIA 500',section:'párr. 9',requirement:'La antigüedad de saldos la produce la entidad: evaluar su exactitud e integridad antes de usarla.'},
 {document:'NIA 505',section:'párr. 7',requirement:'Considerar confirmaciones externas de saldos de clientes como evidencia de existencia.'},
 {document:'NIA 560',section:'párr. 6',requirement:'Revisar los cobros posteriores al cierre como evidencia de recuperabilidad.'},
];
export const catalog={
 vnr:{id:'vnr',name:'Valor neto de realización',area:'Inventarios',description:'Compara el costo con el precio estimado de venta menos los costos de terminación y venta, por partida.',summary:'La NIC 2 exige medir los inventarios al menor entre el costo y el valor neto de realización (párr. 9). El VNR es el precio estimado de venta menos los costos de terminación y los necesarios para la venta (párr. 6). La rebaja se calcula partida por partida (párr. 29) y se revierte si cambian las circunstancias (párr. 33).',source:IAS2,source_pymes:{organization:'IFRS Foundation',document:'NIIF para las PYMES, Sección 13 párr. 13.19 y Sección 27 párr. 27.2–27.4',url:PYMES,type:'Norma contable'},nia:NIA_VNR,frameworks:['NIIF completas','NIIF para las PYMES'],fields:[f('id','Código / lote','text'),f('description','Descripción','text'),f('quantity','Cantidad','number',true),f('unit_cost','Costo unitario'),f('selling_price','Precio de venta unitario'),f('completion_cost','Costo de terminación unitario'),f('selling_cost','Costo necesario de venta unitario'),f('recorded_allowance','Deterioro registrado total')],rules:[rule('net_price','Precio menos terminación','subtract','selling_price','completion_cost',6),rule('nrv_unit','VNR unitario','subtract','net_price','selling_cost',6),rule('nrv_floor','VNR recuperable (mínimo cero)','max','nrv_unit','#0',6),rule('carrying_unit','Menor entre costo y VNR','min','unit_cost','nrv_floor',6),rule('cost','Costo total','multiply','quantity','unit_cost'),rule('recoverable','Valor recuperable total','multiply','quantity','carrying_unit'),rule('impairment','Deterioro requerido','subtract','cost','recoverable'),rule('adjustment','Ajuste frente a deterioro registrado','subtract','impairment','recorded_allowance')],control:'cost',primary:'impairment'},
 pce:{id:'pce',name:'Pérdidas crediticias esperadas',area:'Cuentas por cobrar',description:'Matriz de provisión con tasas y rangos de mora aprobados. Los cobros posteriores se muestran como evidencia, sin descontarlos automáticamente.',summary:'La NIIF 9 permite, para cuentas por cobrar comerciales, medir la corrección por pérdidas por las pérdidas crediticias esperadas durante toda la vida del activo (enfoque simplificado, párr. 5.5.15); la matriz de provisión es un método práctico admitido (párr. B5.5.35). Con NIIF para las PYMES rige otro modelo (pérdida incurrida), por eso esta matriz exige NIIF completas.',source:IFRS9,source_pymes:{organization:'IFRS Foundation',document:'NIIF para las PYMES, Sección 11 párr. 11.21–11.26 (pérdida incurrida)',url:PYMES,type:'Norma contable'},nia:NIA_PCE,frameworks:['NIIF completas'],fields:[f('id','Documento / cliente','text'),f('due_date','Fecha de vencimiento','date'),f('exposure','Exposición al corte'),f('recorded_allowance','Provisión registrada'),f('subsequent_collection','Cobros posteriores (evidencia)')],rules:[rule('impairment','Pérdida estimada','multiply','exposure','rate'),rule('adjustment','Ajuste frente a provisión registrada','subtract','impairment','recorded_allowance')],control:'exposure',primary:'impairment'}
};
