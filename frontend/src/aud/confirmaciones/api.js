...
export async function processConfirmaciones(project,payload){return (await call(project,'process',{method:'POST',body:JSON.stringify(payload),headers:{'Content-Type':'application/json'}})).json()}
export async function downloadConfirmaciones(project,payload,format){const r=await call(project,'download?format='+format,{method:'POST',body:JSON.stringify(payload),headers:{'Content-Type':'application/json'}});saveBlob(await r.blob(),'AuditBrain_Confirmaciones.'+format)}
export async function sendConfirmaciones(project,payload){return (await call(project,'enviar',{method:'POST',body:JSON.stringify(payload),headers:{'Content-Type':'application/json'}})).json()}
