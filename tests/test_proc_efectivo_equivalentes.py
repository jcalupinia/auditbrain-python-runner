...
def test_ejemplo_cifras_a_mano():
    ...
    assert t["reclasRestringido"] == "11650.00"           # 10.000 (garantía) + 1.650 (embargo)
    assert t["reclasNoEquivalentes"] == "30000.00"        # certificado de 180 días
    assert t["auditado"] == "233160.50"                   # 274.630,50 + 180 − 11.650 − 30.000
    assert t["ajuste"] == "-41470.00"
    ...
    assert cu["1.1.03.01"]["califica"] == "Sí" and cu["1.1.03.01"]["plazo"] == 60
...
def test_ya_presentado_aparte_no_se_reclasifica():
    ...
    r = _run(d)
    assert r["totals"]["reclasRestringido"] == "1650.00"
...
def test_parametros():
    ...
    assert {c["id"]: c for c in r["detalle"]["cuentas"]}["1.1.02.02"]["clasif"] == "Corriente"
...
def test_partida_sin_cuenta_y_otra():
    ...
    assert r["totals"]["auditado"] == "233160.50"            # la partida huérfana no toca ninguna cuenta
